...
import {
    ...
    getMyReviewActions,
    myMeetingsActionSelector,
} from '../queries/fusion-meetings-queries';
...
export const sortByDate = (a: Task, b: Task) => {
    ...
    if (
        (a.dueDate === undefined || a.dueDate === '') &&
        (b.dueDate === undefined || b.dueDate === '')
    ) {
...
export function useAssignment() {
    ...
    const { data: queryAndNCRRequests, isLoading: queryAndNCRRequestsLoading } =
        useQueryAndNCRRequestQuery();
...
export function useAssignmentQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            getFusionTasks(await client, signal),
...
export function useProCoSysTaskQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            getProCoSysAssignments(await client, signal),
...
export function usePimsTaskQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) => getPimsTasks(await client, signal),
...
export function useMeetingsActionsQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            getMyMeetingsActions(await client, signal),
...
export function useReviewActionsQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            getMyReviewActions(await client, await contextClient, signal),
...
export function useQueryAndNCRRequestQuery() {
    ...
    return useQuery({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            getQueryAndNCRequest(await client, signal),
        refetchInterval: 5000 * 60,
        staleTime: 2000 * 60,
    });
}
