...
const sortTitleByQuery =
    (query: string) => (a: SearchableDropdownResultItem, b: SearchableDropdownResultItem) => {
        ...
        return {
            searchQuery: async (query: string) => {
                ...
                const matched = alteredAsDropdownItems
                    ?.filter(
                        (item) =>
                            item.title?.match(matcher) ||
                            item.subTitle?.match(matcher) ||
                            item.id?.match(matcher),
                    )
                    .sort(sortByTitle)
